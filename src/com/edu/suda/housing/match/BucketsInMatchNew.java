...
import com.edu.suda.housing.match.BucketsInMatchNew.TempHouseInfo;
import com.edu.suda.housing.tfidf.*;
import com.edu.suda.housing.tools.DBCompute;
import com.edu.suda.housing.tools.IndexDic;
import com.edu.suda.housing.tools.PrintData;
public class BucketsInMatchNew {
...
//		    			}
	...
	//对编码中存在空值的处理
	/*1、根据其他筒的重要信息（可靠的，对应该字段非空）进行填充
	 * 2、根据描述信息进行填充
	 * 3、在1的基础上使用描述信息（具有较高相似适度）进行填充
	 * */
	//将原先的桶根据编码是否有空值分为两大类：含空和不含空
	public void getNullAndNoneBuckets(String sql){
	...
	//使用高相似度（除空之外其他都一样）进行填充
	public boolean inputNullCodeByOthres(){
		double max = 0.0;
		String flag = null;
		boolean refresh = true;
		for(String i: buckets.keySet()){
			if(buckets.get(i).get(0).code.contains("null")){
				for(String j: buckets.keySet()){
					if(!buckets.get(j).get(0).code.contains("null") && i != j){
						double bucketsSim = sim.similarity(buckets.get(i).get(0).code, buckets.get(j).get(0).code);				
					    if(bucketsSim>max){
					    	max = bucketsSim;
					    	flag = j;
					    }
				...
				if(max>EditDistanceSimilarityFunction.SIMILARITY_THRESHOLD){
					for(House h : buckets.get(flag)){
						buckets.get(i).add(h);
						refresh = buckets.get(flag).remove(h);
					}
				}
			}
		}
	...
	public void inputNullCodeByDes(String tableName, String col){
